...
function applyTranslations() {
    if (speechBubble && translations['pet.bark']) {
        // 只有当气泡显示默认文本时才更新
        if (speechBubble.textContent === '汪汪！' || speechBubble.textContent === 'Woof woof!') {
...
function playDefaultBarkSound() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        oscillator.frequency.exponentialRampToValueAtTime(180, startTime + duration);
...
function setExcited(excited) {
    ...
    if (config.customPetEnabled) {
        ...
        normalEyes.forEach(eye => eye.style.display = excited ? 'none' : 'block');
        excitedEyes.forEach(eye => eye.style.display = excited ? 'block' : 'none');
...
function loadCustomSounds() {
    window.electronAPI.getCustomSounds().then(sounds => {
        customSounds = sounds;
    }).catch(err => {
