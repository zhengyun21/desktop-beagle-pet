...
function updateUITexts() {
  ...
  if (!config.customPetEnabled) {
    if (translations["settings.appearance.defaultName"]) {
      elements.currentPetName.textContent =
        translations["settings.appearance.defaultName"];
    }
    if (translations["settings.appearance.defaultType"]) {
      elements.currentPetType.textContent =
        translations["settings.appearance.defaultType"];
    }
  }
...
function updatePetDisplay() {
  ...
  if (config.customPetEnabled && config.customPetPath) {
    ...
    elements.currentPetName.textContent =
      translations["settings.appearance.defaultName"] || "默认比格犬";
    elements.currentPetType.textContent =
      translations["settings.appearance.defaultType"] || "内置SVG";
...
function renderSoundList() {
  ...
  customSounds.forEach((sound, index) => {
    ...
    const selectText =
      index === config.selectedSoundIndex
        ? translations["settings.sounds.selected"] || "默认"
        : translations["settings.sounds.select"] || "设为默认";
    const deleteText = translations["settings.sounds.delete"] || "删除";
    const soundTypeText = translations["settings.sounds.soundType"]
      ? `${ext} ${translations["settings.sounds.soundType"]}`
      : `${ext} 格式`;
...
function toggleCustomSound() {
  ...
  elements.customSoundToggle.classList.toggle(
    "active",
    config.customSoundEnabled,
  );
...
function openReminderModal(reminder = null) {
  ...
  const editTitle =
    translations["settings.reminders.modalEditTitle"] || "编辑提醒";
...
async function saveReminder() {
  ...
  if (!title || !time) {
    const msg =
      translations["settings.reminders.required"] || "请填写完整的提醒信息";
    alert(msg);
    return;
  }
...
async function deleteReminder(id) {
  const confirmText =
    translations["settings.reminders.confirmDelete"] ||
    "确定要删除这个提醒吗？";
